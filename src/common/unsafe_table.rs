...
use crate::common::linked_list::{List, Node, OptNode, CursorMut as ColumnCursorMut};
...
impl<T> Deq<T> {
    pub fn new_with<F, K>(block_capacity: usize, f: F, k: K) -> Self
        where F: Fn() -> T, K: FnMut(&mut T)
    {
        debug_assert_ne!(block_capacity, 0);
        Deq {
            data: vec![UnsafeArray::new_with(block_capacity, f, k)],
            len: 0,
            block_capacity
        }
    }
    ...
    pub fn grow<F, K>(&mut self, f: F, k: K)
        where F: Fn() -> T, K: FnMut(&mut T)
    {
        self.data.push(UnsafeArray::new_with(self.block_capacity, f, k));
    }
    ...
    // pub unsafe fn set(&mut self, index: usize, value: T) -> &mut T {
    //     assert!(index < self.len);
    //     let block_index = index / self.block_capacity;
    //     let inblock_index = index % self.block_capacity;
    //     unsafe {
    //         self.data[block_index].set(inblock_index, value);
    //         self.data[block_index].index_mut(inblock_index)
    //     }
    // }
}
...
pub(crate) struct UnsafeTable<T> {
    array: UnsafeArray<List<T>>,
    nodes: Deq<Node<T>>,
    free_nodes: List<T>,
}

impl<T> UnsafeTable<T> {
    pub fn new<F>(width: usize, capacity: usize, f: F) -> Self
        where F: Fn() -> T
    {
        let mut free_nodes = List::new();

        UnsafeTable {
            array: UnsafeArray::new_with(width, List::new, |_| {}),
            nodes: Deq::new_with(capacity,
                || Node::new(null_mut(), null_mut(), f()),
                |node| free_nodes.push_node_front(node)),
            free_nodes
        }
    }

    pub unsafe fn push_front(&mut self, column: usize, value: T) {
        let mut node = self.free_nodes.pop_node_front();
        if node.is_null() {
